...
- **`following.json` entries carry the username on `title`, not on `string_list_data[].value`.** Verified against a real export (2026-08-11): `relationships_following` entries have no `value` key at all — only `href`/`timestamp` — while `followers_*.json` entries have `value` populated and an empty `title`. `flattenEntries` in `src/lib/parseExport.ts` handles both via `item.value || entry.title`; keep that fallback if you touch parsing. Fixtures under `fixtures/` reflect this real shape now.
- **`followers_*.json`'s bare-top-level-array shape is now verified** (the same real export above used a bare array, not the `{ relationships_followers: [...] }` wrapper). The wrapped-object branch in `parseFollowersJson` is still unverified against a real export — Meta has changed this shape historically, so keep both branches until a real export confirms or removes the wrapper case.
...
- **`RESET` and `ERASE_DATA` are intentionally different, don't merge them.** `RESET` (the "Reset" button) is for loading a different/fresher export and deliberately keeps `triage` — that's the whole point of remembering. `ERASE_DATA` (the "Erase Data" button) is the actual "start over" action: it wipes `triage` too, gated behind a `window.confirm` in `Sidebar.tsx` since it can't be undone.
